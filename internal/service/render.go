...
func plist(cfg Config, _ string) (string, error) {
	...
	writeString("Label", Label(cfg.Component))
...
func unitFile(cfg Config) string {
	args := strings.Join(commandLine(cfg), " ")

	var b strings.Builder
	b.WriteString("[Unit]\n")
	fmt.Fprintf(&b, "Description=je %s\n", cfg.Component)
...
// right, and having launchd and systemd each build it separately is how they
// drift into starting subtly different processes -- the kind of difference you
// find months later on the machine you look at least.
//
// Global flags come before the subcommand and component flags after it, which
// is the CLI's own grammar (see internal/cli): `je --data-dir X worker run
// --labels macos`.
func commandLine(cfg Config) []string {
	argv := []string{cfg.Binary}
	if cfg.DataDir != "" {
		argv = append(argv, "--data-dir", cfg.DataDir)
	}
	argv = append(argv, string(cfg.Component), "run")
