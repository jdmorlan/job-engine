...
func TestAnEnrolledWorkerCannotChooseItsOwnLabels(t *testing.T) {
	...
	if _, _, err := e.Enrol(ctx, token, publicKeyPEM(t)); err != nil {
		t.Fatal(err)
	}
	...
	if len(registered.Labels) != 1 || registered.Labels[0] != "macos" {
		t.Errorf("labels = %v, want [macos] as decided at enrolment", registered.Labels)
	}
...
// A worker that never enrolled keeps working exactly as before. Enrolment is
...
func TestAnIssuedCertificateNamesTheEnrolledWorker(t *testing.T) {
	...
	certPEM, caPEM, err := e.Enrol(ctx, token, publicKeyPEM(t))
	if err != nil {
		t.Fatal(err)
	}
...
func TestATokenCannotBeRedeemedTwice(t *testing.T) {
	...
	_, _, err = e.Enrol(ctx, token, publicKeyPEM(t))
	if err == nil {
...
func TestEnrolmentIsRecordedAsAnEvent(t *testing.T) {
	ctx := context.Background()
	e, _ := chainFixture(t, nil, nil)
